...
describe("Employee routes (part 1)", () => {
    ...
    test("POST /api/v1/employees -> 201", async () => {
        const res = await (0, supertest_1.default)(app_1.default).post("/api/v1/employees").send({
    ...
    test("GET /api/v1/employees/:id -> 200", async () => {
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/v1/employees/${createdId}`);
        expect(res.status).toBe(200);
        expect(res.body.data.id).toBe(createdId);
    });
    test("GET /api/v1/employees -> 200", async () => {
        const res = await (0, supertest_1.default)(app_1.default).get("/api/v1/employees");
        expect(res.status).toBe(200);
...
test("GET /api/v1/employees/abc -> 400", async () => {
    const res = await (0, supertest_1.default)(app_1.default).get("/api/v1/employees/abc");
    expect(res.status).toBe(400);
});
test("PUT /api/v1/employees/:id -> 200 (partial update)", async () => {
    const created = await (0, supertest_1.default)(app_1.default).post("/api/v1/employees").send({
        name: "Patch Me", position: "Analyst", department: "Finance",
        email: "patch@pixell-river.com", phone: "204-555-1111", branchId: 1
    });
    const id = created.body.data.id;
    const res = await (0, supertest_1.default)(app_1.default).put(`/api/v1/employees/${id}`).send({ phone: "204-555-4321" });
    expect(res.status).toBe(200);
    expect(res.body.data.phone).toBe("204-555-4321");
});
test("PUT /api/v1/employees/xyz -> 400", async () => {
    const res = await (0, supertest_1.default)(app_1.default).put("/api/v1/employees/xyz").send({ phone: "123" });
    expect(res.status).toBe(400);
});
test("DELETE /api/v1/employees/:id -> 200 then 404", async () => {
    ...
    const id = created.body.data.id;
    const del1 = await (0, supertest_1.default)(app_1.default).delete(`/api/v1/employees/${id}`);
    expect(del1.status).toBe(200);
    const del2 = await (0, supertest_1.default)(app_1.default).delete(`/api/v1/employees/${id}`);
