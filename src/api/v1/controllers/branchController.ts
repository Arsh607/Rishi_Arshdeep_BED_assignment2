...
export const getAll = (_req: Request, res: Response) =>
  res.status(200).json(successResponse(service.listBranches()));

export const getById = (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (Number.isNaN(id)) return res.status(400).json({ status: "error", message: "Invalid id" });
  const item = service.getBranchById(id);
  if (!item) return res.status(404).json({ status: "error", message: "Branch not found" });
  return res.status(200).json(successResponse(item));
};

export const create = (req: Request, res: Response) => {
  const { name, address, phone } = req.body || {};
  if (!name || !address || !phone) return res.status(400).json({ status: "error", message: "Missing required fields" });
...
export const remove = (req: Request, res: Response) => {
  ...
  if (!ok) return res.status(404).json({ status: "error", message: "Branch not found" });
